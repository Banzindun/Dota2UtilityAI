...
import cz.cuni.mff.kocur.considerations.ConsiderAbilityAttackRange;
...
public class SniperAbilitiesDS extends DecisionSet {
	...
	public void createDecisions() {
		...
		Decision shrapnel = DecisionBuilder.build().setDecision(new CastShrapnel()).setName("CastShrapnel")
				...
				.addConsideration(new ConsiderAbilityAttackRange(), new BinaryFunction(-1, 0.5, 0.9, 0.5))
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, 2000) // 1800 is the range
		...
		Decision asssasinate = DecisionBuilder.build().setDecision(new CastAssasinate()).setName("CastAssasinate")
				...
				.addConsideration(new ConsiderCooldown(), new BinaryFunction(0.5, -1, 0.01, 0.5))
				// Consider attack range
				.addConsideration(new ConsiderAbilityAttackRange(), new BinaryFunction(-1, 0.5, 0.6, 0.5))
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, 5000) // 3000 is the range
